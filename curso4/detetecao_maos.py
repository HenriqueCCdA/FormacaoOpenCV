...
while True:
    ...
    if len(todas_maos) == 1:
        ...
        if todas_maos[0]['lado'] == 'Right':
            if info_dedos_mao1 == [True, False, False, False] and not bloco_notas:
                bloco_notas = True
                subprocess.Popen('/usr/bin/gedit')

            if info_dedos_mao1 == [True, True, False, False] and not chrome:
                chrome = True
                subprocess.Popen('firefox')

            if info_dedos_mao1 == [True, True, True, False] and not calculadora:
                calculadora = True
...
cv2.imwrite('quaddro.png', img_quadro)
